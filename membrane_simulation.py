...
# material constants
_MU_elastic = 31_700 #ones given by Greg
#_MU_elastic = 28_000
_MU_stiff = 1e6
#_JM_elastic = 65
_JM_elastic = 39.6 #ones given by Greg
_JM_stiff = 25
#_JM_stiff = 1
...
_ode_solver = Tsit5() # TODO this solver can probably be substituted w/ something faster (but less accurate)
#_ode_dr0 = 0.0001
_ode_dr0 = 0.000075
...
# setup for evolving DOE using implicit method
_ode_term = ODETerm(vector_field)
_ode_solver = diffrax.Kvaerno5() # TODO this solver can probably be substituted w/ something faster (but less accurate)
_ode_dr0 = 0.0001
_ode_stepsize_controller = diffrax.PIDController(rtol=1e-2, atol=1e-1)
...
def solve_ode(y0, p, r_init, r_end, mat_constants):
    ...
    args = mat_constants, p
    saveat = SaveAt(ts=jnp.linspace(r_init, r_end, 1000))
    sol = diffeqsolve(_ode_term, _ode_solver, r_init, r_end, _ode_dr0, y0, args=args,
                      saveat=saveat, stepsize_controller=_ode_stepsize_controller)
    return sol
'''
...
def simulate_height(x, F, p, h, change_material, material_type):
    ...
    for sol in full_sols:
        ...
        #z = (r_end-r_init)*jnp.cumsum(sol.ys[:,0]*jnp.sin(sol.ys[:,2]))/1000
        i = i+1
        start = start + z[-1]
    return start
